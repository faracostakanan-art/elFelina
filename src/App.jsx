...
export default function App() {
  ...
  const loadOrders = async () => {
    try {
      const res = await fetch(`http://localhost:3001/api/orders/${USER_ID}`);
      const data = await res.json();
  ...
  const openOrder = async (orderId) => {
    try {
      const res = await fetch(`http://localhost:3001/api/orders/${USER_ID}/${orderId}`);
      const data = await res.json();
  ...
  return (
    <div className="app-shell">
      <div className="app-header">
        ...
        <div className="app-subtitle">Interface sécurisée</div>
      </div>
      ...
      <div className="bottom-nav">
        ...
        <div
          className={page === "refund" ? "nav-item active-nav" : "nav-item"}
          onClick={() => {
            setPage("refund");
            setSelectedOrder(null);
            setMessage("");
        ...
        >
          <div className="nav-icon">↩</div>
          <div className="nav-text">Refund</div>
        </div>
      </div>
    </div>
  );
